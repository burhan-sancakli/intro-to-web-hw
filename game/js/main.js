...
function get_character_stats(character,by_unit){
    //şöyle de yapabiliyoz character_stats.s_0_0.hp;
    let variation=0;
    if(by_unit){
...
function spawn_queue(character,side){ //to be implemented
    console.log('spawn');
...
function update_units(){
    // units.forEach(side_units => {
    //     let units_last_index=side_units.length-1;
    //     for(let i = units_last_index;i>=0;i--){
    //         let unit=side_units[i];
    //         if(check_is_died(unit)){
    //             console.log('I SHALL DIEE');
    //         }
    //     }
    // });
}
